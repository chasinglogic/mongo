...
def make_deb_repo(repo, distro, build_os):
    ...
    try:
        dirs = set(
            [os.path.dirname(deb)[2:] for deb in backtick(["find", ".", "-name", "*.deb"]).decode('utf-8').split()])
...
def write_debian_changelog(path, spec, srcdir):
    ...
    try:
        sb = preamble + backtick(
            ["sh", "-c",
             "git archive %s debian/changelog | tar xOf -" % spec.metadata_gitspec()]).decode('utf-8')
    finally:
        os.chdir(oldcwd)
    lines = sb.split("\n")
...
def make_rpm(distro, build_os, arch, spec, srcdir):  # pylint: disable=too-many-locals
    ...
    macrofiles = [
        l for l in
        backtick(["rpm", "--showrc"]).decode('utf-8').split("\n")
